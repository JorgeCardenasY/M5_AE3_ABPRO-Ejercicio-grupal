...
-- 1. Agregamos una restricción ENUM simple a la tabla Articulos
ALTER TABLE Articulos 
MODIFY COLUMN estado ENUM('Pendiente', 'Reciclado', 'En reparación') NOT NULL DEFAULT 'Pendiente';

-- 2. Creamos un procedimiento almacenado básico para manejar la transacción
DELIMITER //
-- Actividad Grupal M5_AE3_ABPRO
-- Desarrollado por: Jorge Cárdenas, Emily Quintana
...
CREATE PROCEDURE TransaccionBasica()
BEGIN
    ...
    INSERT INTO Articulos (tipo_articulo, estado, id_cliente) 
    VALUES ('Monitor', 'Pendiente', 1);
    
    INSERT INTO Articulos (tipo_articulo, estado, id_cliente) 
    VALUES ('Teclado', 'EstadoInvalido', 1); 
...
CALL TransaccionBasica();


SELECT * FROM Articulos WHERE tipo_articulo IN ('Monitor', 'Teclado');